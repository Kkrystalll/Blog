...
![Docker run](/image/dockerDay5/5_1.webp)
...
![Docker run 自動下載 latest](/image/dockerDay5/5_2.webp)
...
![看我目前運行中的 container](/image/dockerDay5/5_3.webp)

結果怎麼是空的？？？我到底有沒有成功執行啊？
我們先使用 `docker ps -a` 指令來看一下所有 container 的狀態
![看一下所有 container 的狀態](/image/dockerDay5/5_4.webp)
...
執行後我們可以看到實際上是進到容器裡面
![進到容器裡面](/image/dockerDay5/5_5.webp)

所以我們可以在這個有 ruby 環境的容器裡進到 irb ，並執行 ruby 語法
![容器裡進到 irb](/image/dockerDay5/5_6.webp)

當這時我們再開一個新的終端機，使用 `docker ps` 就可以看到，我現在正在運行的 container 了

![docker ps](/image/dockerDay5/5_7.webp)
...
![命名我的 container](/image/dockerDay5/5_8.webp)
...
![在容器裡停止](/image/dockerDay5/5_9.webp)
...
![刪除運行中的 container](/image/dockerDay5/5_10.webp)
